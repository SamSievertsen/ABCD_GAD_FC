...
KSADS_variable_names <- read.csv("data_raw/supplementary_ksads_variable_desc.csv")

# QC'd + cleaned imaging data
...
ABCD_KSADS_release_5.1_data <- ABCD_KSADS_release_5.1_parent_report
...
ABCD_KSADS_release_5.1_data <- ABCD_KSADS_release_5.1_data %>%
  mutate_at(vars(3:111), as.numeric)
...
#1.2 Subset control subjects who have imaging data passing QA/QC
healthy_control_group <- inner_join(healthy_control_group,
                                    cleaned_qcd_imaging_data,
                                    by = "src_subject_id") %>%
  dplyr::select(src_subject_id, eventname)

#1.3 Add the group variable
