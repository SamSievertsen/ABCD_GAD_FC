...
#2. Create violin boxplots for each of the dv of interest
...
for (dv in post_hoc_dependent_variables) {
  ...
  plot <- ggplot(comorbid_clinical_HC_group_connectivity_analysis_data, 
                 aes(x = comorbidity_group, y = .data[[dv]], fill = comorbidity_group)) +
    geom_violin(trim = FALSE, alpha = 0.6, color = "black") +
    geom_boxplot(width = 0.2, outlier.shape = NA, alpha = 0.9) +
    labs(
      title = paste("Violin Boxplot for", dv),
      x = "Comorbidity Group",
      y = dv
    ) +
    theme_minimal() +
  ...
  violin_plots[[dv]] <- plot
}

# Display each plot
for (dv in post_hoc_dependent_variables) {
  print(violin_plots[[dv]])
}
