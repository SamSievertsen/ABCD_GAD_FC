...
#1. Create a dataframe specific to the HC and GAD groups
#1.1 GAD group
GAD_group <- GAD_HC_group_not_resampled %>% 
  filter(group == "GAD")

#1.2 HC group
HC_group_not_resampled <- GAD_HC_group_not_resampled %>% 
  filter(group == "HC")

#2. Determine if there are any subjects who have data at multiple sites, and if so, randomly sample the data point to keep for each subject
#2.1 Identify subjects who have different site_name values across different eventname's
...
#2.12 Randomly select one data point for each subject with assessment data at multiple sites, since all multi site subjects have both baseline and 2 year follow up data
#2.121 Initialize an empty dataframe 
multi_site_selected_data <- data.frame()

#2.122 Loop through the multi-site subjects and store their sampled data point
for (subject in multi_site_subjects) {
  
  #2.1221 Subset the data for the current subject
  subject_data <- subset(HC_group_not_resampled, src_subject_id == subject)
  
  #2.1222 Randomly select either the baseline or followup data point with a 50% probability of each
  if (runif(1) > 0.5) {
...
#2.13 Replace the existing multi site subject data with the resampled data 
#2.131 Remove the current multi site subject data from the HC group 
HC_group_cleaned_not_resampled <- HC_group_not_resampled %>%
  filter(!(src_subject_id %in% multi_site_subjects))

#2.132 Add back in the selected multi site subject data
HC_group_cleaned_not_resampled <- full_join(HC_group_cleaned_not_resampled, multi_site_selected_data)

#3. Generate summary statistics outlining the distribution of GAD + HC subjects at each combination of site and eventname
#3.1 Create a dataframe containing the N and % of subjects at each combination of site and eventname, with the % of subjects at each timepoint within site rounded to 2 decimal places for purposes of maximizing the sample during matching
GAD_site_visit_distribution <- GAD_group %>% 
  group_by(site_name, eventname) %>%
  summarize(GAD_site_visit_n = n()) %>%
  group_by(site_name) %>%
  mutate(GAD_site_n = sum(GAD_site_visit_n),
         GAD_site_pct = round((GAD_site_visit_n / GAD_site_n), digits = 2))

#3.2 Likewise for the HC distribution, create a dataframe containing the N and % of subjects at each combination of site and eventname, with the % of subjects at each timepoint within site rounded to 2 decimal places for purposes of maximizing the sample during matching
...
#3.3 Merge the HC + GAD distributions at each site-visit combination in the GAD data
#3.31 Combine the GAD + HC distribution data
merged_HC_GAD_site_visit_distribution <- merge(GAD_site_visit_distribution, HC_site_visit_distribution, by = c("eventname", "site_name"), all.x =  TRUE)

#3.32 Add a new variable to the merged distribution stats that contains the maximum possible number of controls for each combination of site + eventname based on the GAD distribution and N controls
merged_HC_GAD_site_visit_distribution <- merged_HC_GAD_site_visit_distribution %>% 
...
resample_healthy_controls <- function(site_id, HC_group_data, merged_HC_GAD_distribution, GAD_group_data) {
  ...
  merged_HC_GAD_site_visit_distribution <-
    merge(
      merged_HC_GAD_distribution,
  ...
  healthy_control_baseline_only <- merged_HC_GAD_site_visit_distribution %>%
    group_by(src_subject_id) %>%
    filter(all(eventname == "baseline_year_1_arm_1") & n() == 1)
  
  #1.132 Followup subjects only (no baseline data)
  healthy_control_followup_only <- merged_HC_GAD_site_visit_distribution %>%
    group_by(src_subject_id) %>%
  ...
  num_unique_control_subs <- length(unique(merged_HC_GAD_site_visit_distribution$src_subject_id))
  
  #1.21 Subset GAD subjects for this site
  GAD_subs <- GAD_group_data %>%
    filter(site_name == site_id)
  
  #1.22 Store the total number of GAD subjects at this site
  total_GAD_sub_n <- length(unique(GAD_subs$src_subject_id))
  
  #1.31 Further subset the GAD data into baseline and follow-up subjects
  baseline_GAD_data <- GAD_subs %>% filter(eventname == "baseline_year_1_arm_1")
  followup_GAD_data <- GAD_subs %>% filter(eventname == "2_year_follow_up_y_arm_1")
  
  #1.32 Calculate the GAD percentage distribution for baseline and follow up
  #1.321 Baseline GAD percentage distribution
  baseline_GAD_site_visit_percent <- merged_HC_GAD_site_visit_distribution %>% 
    filter(site_name == site_id & eventname == "baseline_year_1_arm_1") %>%
    pull(GAD_site_pct) %>% 
    unique()
  
  #1.322 Follow up GAD percentage distribution
  followup_GAD_site_visit_percent <- merged_HC_GAD_site_visit_distribution %>%
  ...
  #1.62 Calculate the desired N of controls at both baseline and followup based on the GAD distribution
  baseline_controls_desired <- floor(num_unique_control_subs * baseline_GAD_site_visit_percent)
  followup_controls_desired <- floor(num_unique_control_subs * followup_GAD_site_visit_percent)
  ...
  adjusted_total_control_n_if_insufficient_baseline <-
    floor(max_baseline_controls / baseline_GAD_site_visit_percent)
  ...
  adjusted_total_control_n_if_insufficient_followup <-
    ifelse(
      followup_GAD_site_visit_percent > 0,
      floor(max_followup_controls / followup_GAD_site_visit_percent), 0)
  ...
  if (!sufficient_baseline_control_n & sufficient_followup_control_n) {
    ...
    updated_control_counts$num_followup_controls <-
      floor(adjusted_total_control_n_if_insufficient_baseline * followup_GAD_site_visit_percent)
  ...
  #1.662 If not enough follow-up controls, set to max and adjust baseline accordingly
    ...
    updated_control_counts$num_baseline_controls <-
      floor(adjusted_total_control_n_if_insufficient_followup * baseline_GAD_site_visit_percent)
    updated_control_counts$num_followup_controls <-
...
for (site in unique_sites) {
  healthy_control_sample <- resample_healthy_controls(site, HC_group_cleaned_not_resampled, merged_HC_GAD_site_visit_distribution, GAD_group)
...
healthy_control_sample_resampled <- combined_healthy_control_samples %>% 
  mutate(group = rep("HC"),
         analysis_group = rep("HC"),
         subgroup = rep("HC"))

#2.42 Add back in the demographic information 
healthy_control_sample_resampled <- left_join(healthy_control_sample_resampled, merged_GAD_HC_sample_data_cleaned)

#3. Merge the GAD group with the newly sampled HC group
GAD_HC_sample <- full_join(GAD_group, healthy_control_sample_resampled)
...
#1.21 Create a dataframe comparing the distribution of GAD subjects and sampled controls at each site-event combination (both N and % distribution)
resampled_HC_site_visit_distribution_comparison <- merge(GAD_site_visit_distribution, resampled_HC_site_visit_distribution, by = c("site_name", "eventname"), all = TRUE)

#1.22 Add a column that tests whether the GAD distribution matches the sampled control subject distribution (allowing for a 0.01 difference due to rounding error by r's "round" function)
resampled_HC_site_visit_distribution_comparison$distribution_converged <- 
  ifelse(
    abs(round(resampled_HC_site_visit_distribution_comparison$control_site_pct, 2) - 
          round(resampled_HC_site_visit_distribution_comparison$GAD_site_pct, 2)) < 0.011,
    "Success", "Debug")

#1.3 Check whether the sampled control distributions at each site-event combination and whether that matches the distribution of the GAD subjects, and print a message detailing whether those were successful or not
print(
  if_else(
...
#2.11 Check whether the sampled control distributions at each site-event combination and whether that matches the distribution of the GAD subjects
